...
function App() {
      ...
      if (reference && window.valCarePaymentApiUrl) {
        try {
          ...
          if (orderSaved) {
            ...
            setOrderConfirmationId(String(orderId));
          }
          setOrderPlaced(true);
          setActivePanel("cart");
          if (orderSaved) {
  ...
  useEffect(() => {
    if (!(["reviews", "product-preview"].includes(activePanel)) || !selectedProduct || !window.valCareDb) return undefined;
    let cancelled = false;
  ...
  useEffect(() => {
    if (activePanel !== "transactions" || isAdmin || !window.valCareDb) return undefined;
    const user = window.valCareAuth?.currentUser;
    if (!user) {
      setCustomerOrders([]);
      return undefined;
    ...
    const loadOrders = async () => {
      try {
        let snapshot;
        try {
          snapshot = await window.valCareDb.collection("orders").where("userId", "==", user.uid).orderBy("createdAt", "desc").limit(50).get();
        } catch {
          snapshot = await window.valCareDb.collection("orders").where("userId", "==", user.uid).limit(50).get();
        }
        if (cancelled) return;
  ...
  const openProductPreview = (product) => {
    ...
    setReviews([]);
    setReviewMessage("");
    setActivePanel("product-preview");
  };
          ...
          <div className="product-grid">
            {filteredProducts.map((product) => (
              <article className="product-card" key={product.id}>
                <div className={`product-image ${product.tone}`}><button className="product-image-trigger" type="button" onClick={() => openProductPreview(product)} aria-label={`View ${product.name} details`}>{product.imageUrl ? <img src={product.imageUrl} alt={`${product.name} preview`} /> : <span>{product.icon}</span>}{product.tag && <span className="badge">{product.tag}</span>}</button>{userName && !isAdmin && <button className={`favorite-button ${favoriteIds.includes(String(product.id)) ? "active" : ""}`} type="button" onClick={() => toggleFavorite(product)} aria-label={favoriteIds.includes(String(product.id)) ? `Remove ${product.name} from favorites` : `Save ${product.name} to favorites`}>{favoriteIds.includes(String(product.id)) ? "♥" : "♡"}</button>}</div>
      ...
      {activePanel && ReactDOM.createPortal(<div className="panel-backdrop" onClick={closePanel}>
        <aside className={`account-panel theme-${theme}`} onClick={(event) => event.stopPropagation()}>
          ...
          {activePanel === "cart" && !isAdmin && <div className="panel-content">
            {orderPlaced && <div className="success-message"><strong>Order received successfully.</strong><p>Copy your order ID and send it to Val’s Glam on WhatsApp, or wait for our team to contact you.</p>{orderConfirmationId && <div className="order-confirmation"><code>{orderConfirmationId}</code><button type="button" onClick={() => copyOrderId(orderConfirmationId)}>Copy order ID</button><a href={`https://wa.me/233594972748?text=${encodeURIComponent(`Hello Val’s Glam, my order ID is ${orderConfirmationId}.`)}`} target="_blank" rel="noreferrer">WhatsApp Val’s Glam</a></div>}</div>}
            ...
            {!!cartItems.length && <>
              <div className="checkout-instructions"><strong>Delivery details required</strong><span>Enter these three details before payment so Val’s Glam can deliver your order:</span><ol><li>Your WhatsApp or phone contact</li><li>Your location or town</li><li>Your GPS address or Google Maps address</li></ol></div>
          ...
          {activePanel === "transactions" && <div className="panel-content transaction-list">{isLoadingTransactions ? <div className="panel-empty"><LoadingSpinner label="Loading orders" /></div> : customerOrders.length ? customerOrders.map((order) => { const orderDate = order.createdAt?.toDate?.(); const status = String(order.status || "paid").toLowerCase(); return <div className="transaction-card" key={order.id}><div><strong>{order.id}</strong><span>{orderDate ? orderDate.toLocaleDateString() : "Recently placed"} · {order.items?.length || 0} item(s)</span></div><strong>{formatPrice(Number(order.total || 0) / (currencies[order.currency]?.rate || 1))}</strong><em className={status === "delivered" ? "transaction-delivered" : "transaction-pending"}>{status === "delivered" ? "Delivered" : "Pending delivery"}</em></div>; }) : <div className="panel-empty"><p>Your purchases will appear here after checkout.</p></div>}</div>}
          ...
          {activePanel === "product-preview" && selectedProduct && <div className="panel-content product-preview-panel"><div className={`product-preview-image ${selectedProduct.tone}`}>{selectedProduct.imageUrl ? <img src={selectedProduct.imageUrl} alt={selectedProduct.name} /> : <span>{selectedProduct.icon}</span>}</div><div className="product-preview-copy"><div><p className="eyebrow">{selectedProduct.category}</p><h3>{selectedProduct.name}</h3></div><strong className="product-preview-price">{formatPrice(selectedProduct.price)}</strong></div><div className="product-preview-stats"><span><b>{Number(selectedProduct.stock) <= 0 ? "Sold out" : selectedProduct.stock}</b><small>Stock</small></span><span><b>{reviews.length ? (reviews.reduce((total, review) => total + Number(review.rating || 0), 0) / reviews.length).toFixed(1) : "-"}</b><small>Rating</small></span><span><b>{reviews.length}</b><small>Reviews</small></span></div><button className="settings-save" type="button" onClick={() => openReviews(selectedProduct)}>Read reviews</button>{!isAdmin && <button className="add-button product-preview-add" type="button" onClick={() => { addToBag(selectedProduct); closePanel(); }} disabled={Number(selectedProduct.stock) <= 0}>{Number(selectedProduct.stock) <= 0 ? "Out of stock" : `+ ${text.add}`}</button>}</div>}
